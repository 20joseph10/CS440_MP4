...
class NN(object):
    def __init__(self, n_layer=4, n_units=256, learn_rate=0.1, File_name = None):
        ...
        #     train_set.append(i.strip().split())
        #     num += 1
        # tag.append(len(train_set))
        # # train_set = np.array(train_set).astype('float')
        # test = []
        # for i in range(int(len(tag) / 2)):
        #     test.append(np.array(train_set[tag[2 * i] + 1:tag[2 * i + 1]]).astype('float'))
    ...
    def train(self, epoch, init_weight):
        ...
        save_file_name = 'after_train.txt'
        with open(save_file_name, 'w') as ff:
            for i in range(len(self.weights)):
                ff.write('L' + str(i) + '\n')
                ff.write(str(self.weights[i]) + '\n')
                ff.write('\n')
                ff.write(str(self.biases[i]) + '\n')
        ff.close()
